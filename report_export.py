...
class ReportExportService:
    ...
    def _generate_pdf_report(self, data: Dict[str, Any], days: int) -> io.BytesIO:
        ...
        # Titre principal
        title = Paragraph("📊 Rapport Analytics YouTube Transcript Analyzer", 
    ...
    def _create_kpis_section(self, kpis: Dict[str, Any]) -> List:
        ...
        elements.append(Paragraph("📈 Indicateurs Clés de Performance (KPIs)", 
        ...
        kpi_data = [
            ...
            ['Analyses (période)', f"{kpis['totals'].get('analyses_period', 0):,}", 'Analyses effectuées sur la période'],
            ['Analyses (total)', f"{kpis['totals'].get('analyses_ever', 0):,}", 'Total depuis le début'],
            ['Mots analysés', f"{kpis['totals'].get('words_analyzed', 0):,}", 'Mots traités au total'],
            ['Temps de lecture', f"{kpis['totals'].get('reading_time_hours', 0):.1f}h", 'Heures de contenu analysé'],
        ]
        
        if kpis.get('averages'):
            kpi_data.extend([
                ['Complexité moyenne', f"{kpis['averages'].get('complexity_score', 0):.3f}", 'Score de difficulté moyen'],
                ['Sentiment moyen', f"{kpis['averages'].get('sentiment_polarity', 0):.3f}", 'Polarité sentiment moyenne'],
                ['Analyses/jour', f"{kpis['averages'].get('analyses_per_day', 0):.1f}", 'Moyenne quotidienne'],
            ])
        ...
        if kpis.get('distributions'):
            elements.append(self._create_distributions_section(kpis['distributions']))
    ...
    def _create_trends_section(self, trends: Dict[str, Any]) -> List:
        ...
        if not trends or not trends.get('daily_analyses'):
            elements.append(Paragraph("Aucune donnée de tendance disponible", self.styles['Normal']))
            return elements
        ...
        if daily_analyses:
            ...
            max_day = max(daily_analyses, key=lambda x: x['count'], default={})
    ...
    def _create_insights_section(self, insights: Dict[str, Any]) -> List:
        ...
        elements.append(Paragraph("🧠 Insights et Métriques Avancées", self.styles['CustomHeading2']))
        elements.append(Spacer(1, 12))
        ...
        if insights.get('top_analyses'):
            elements.append(self._create_top_analyses_section(insights['top_analyses']))
    ...
    def _create_top_analyses_section(self, top_analyses: Dict[str, Any]) -> List:
        ...
        if top_analyses.get('most_words'):
            ...
            if top_words:
                elements.append(Paragraph(f"• Plus de mots: {top_words['words']:,} mots (Analyse #{top_words['id']})", 
                                        self.styles['Normal']))
        ...
        if top_analyses.get('most_complex'):
            ...
            if top_complex:
                elements.append(Paragraph(f"• Plus complexe: {top_complex['complexity']:.3f} (Analyse #{top_complex['id']})", 
                                        self.styles['Normal']))
        
        # Richest vocabulary
        if top_analyses.get('richest_vocabulary'):
            top_vocab = top_analyses['richest_vocabulary'][0] if top_analyses['richest_vocabulary'] else None
            if top_vocab:
                elements.append(Paragraph(f"• Vocabulaire le plus riche: {top_vocab['richness']:.3f} (Analyse #{top_vocab['id']})", 
                                        self.styles['Normal']))
    ...
    def _create_excel_kpis_sheet(self, ws, kpis: Dict[str, Any]):
    ...
    def _create_excel_trends_sheet(self, ws, trends: Dict[str, Any]):
    ...
    def _create_excel_insights_sheet(self, ws, insights: Dict[str, Any]):
