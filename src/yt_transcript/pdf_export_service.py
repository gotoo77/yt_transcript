...
class PDFExportService:
    ...
    def create_sentiment_chart(self, analysis: Analysis) -> io.BytesIO | None:
    ...
    def create_metrics_chart(self, analysis: Analysis) -> io.BytesIO | None:
        ...
        try:
            ...
            for i, (key, value) in enumerate(metrics.items()):
                ax1.text(
                    i, value + max(metrics.values()) * 0.01, str(value), ha="center", va="bottom"
                )

            # Graphique 2: Scores de qualité
            scores = {
                "Richesse vocab.": (analysis.vocabulary_richness or 0) / 100,
                "Complexité": (analysis.complexity_score or 0) / 200,
                "Flesch Ease": (analysis.flesch_reading_ease or 0) / 100,  # Normaliser sur 1
            }

            bars = ax2.bar(scores.keys(), scores.values(), color=["gold", "orange", "lightcoral"])
            ax2.set_title("Scores de Qualité")
            ax2.set_ylabel("Score (normalisé)")
            ax2.set_ylim(0, 1)

            # Ajouter les valeurs
            for bar, (key, score) in zip(bars, scores.items()):
                height = bar.get_height()
                if key == "Flesch Ease":
                    display_value = f"{score * 100:.1f}"
                else:
    ...
    def export_analysis_to_pdf(
        self, analysis_id: int, output_path: str | None = None
    ) -> str | None:
        ...
        try:
            ...
            if not output_path:
                ...
                directory = Path(current_app.config["DATA_DIR"]) / "exports"
                directory.mkdir(parents=True, exist_ok=True)
                output_path = str(directory / f"analysis_{analysis_id}_{timestamp}.pdf")
            ...
            # 1. Titre principal
            title = Paragraph(f"Rapport d'Analyse - ID #{analysis.id}", self.styles["CustomTitle"])
            story.append(title)
            ...
            if analysis.summary_text:
                ...
                summary_text = analysis.summary_text[:1000] + (
                    "..." if len(analysis.summary_text) > 1000 else ""
                )
