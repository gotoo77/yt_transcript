...
from .database import Analysis, get_analysis_by_id, get_recent_analyses
...
class TagsService:
    def __init__(self) -> None:
        ...
        self.predefined_tags: dict[str, dict[str, dict[str, Any]]] = {
            "complexity": {
    ...
    def auto_tag_analysis(self, analysis: Analysis) -> list[str]:
        ...
        try:
            ...
            if analysis.sentiment_polarity is not None:
                for tag_key, criteria in self.predefined_tags["sentiment"].items():
                    if (
                        criteria["min_polarity"]
                        <= analysis.sentiment_polarity
                        <= criteria["max_polarity"]
                    ):
            ...
            if analysis.created_at:
                ...
                auto_tags.extend([f"year:{year}", f"month:{month}", f"weekday:{weekday}"])
    ...
    def add_custom_tag(
        self, analysis_id: int, tag_name: str, tag_value: str | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "analysis_id": analysis_id, "tag_added": tag_entry}

        except Exception as e:
            logger.error(f"Erreur ajout tag: {e}")
            return {"error": f"Erreur lors de l'ajout du tag: {str(e)}"}

    def get_analysis_tags(self, analysis_id: int) -> dict[str, Any]:
    ...
    def filter_analyses_by_tags(
        self, tag_filters: dict[str, Any], limit: int = 50
    ) -> dict[str, Any]:
    ...
    def get_tag_statistics(self, limit: int = 100) -> dict[str, Any]:
        ...
        try:
            ...
            monthly_distribution: defaultdict[str, int] = defaultdict(int)
            for analysis in analyses:
                if analysis.created_at:
    ...
    def suggest_tags(self, analysis_id: int) -> list[dict[str, Any]] | dict[str, Any]:
    ...
    def create_tag_hierarchy(self, limit: int = 100) -> dict[str, Any]:
        """Crée une hiérarchie des tags disponibles"""
        try:
            analyses = get_recent_analyses(limit=limit)

            hierarchy: dict[str, Any] = {"categories": {}, "total_analyses": len(analyses)}
    ...
    def export_tagged_analyses(
        self, tag_filters: dict[str, Any] | None = None, output_format: str = "json"
    ) -> str | dict[str, Any]:
