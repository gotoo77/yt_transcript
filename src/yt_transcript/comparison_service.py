...
from .database import Analysis, get_analysis_by_id
...
class ComparisonService:
    ...
    def _get_analyses_overview(self, analyses: list[Analysis]) -> list[dict[str, Any]]:
            ...
            overview.append(
                {
                    ...
                    "summary_preview": (analysis.summary_text or "")[:100]
                    + ("..." if analysis.summary_text and len(analysis.summary_text) > 100 else ""),
                }
            )

        return overview

    def _compare_metrics(self, analyses: list[Analysis]) -> dict[str, Any]:
        ...
        for analysis in analyses:
            metrics["text_metrics"]["total_words"].append(
                {"id": analysis.id, "value": analysis.total_words or 0}
            )
            metrics["text_metrics"]["unique_words"].append(
                {"id": analysis.id, "value": analysis.unique_words or 0}
            )
            metrics["text_metrics"]["sentences"].append(
                {"id": analysis.id, "value": analysis.sentences or 0}
            )
            metrics["text_metrics"]["text_length"].append(
                {"id": analysis.id, "value": analysis.text_length or 0}
            )

            metrics["quality_metrics"]["vocabulary_richness"].append(
                {"id": analysis.id, "value": analysis.vocabulary_richness or 0}
            )
            metrics["quality_metrics"]["complexity_score"].append(
                {"id": analysis.id, "value": analysis.complexity_score or 0}
            )
            metrics["quality_metrics"]["reading_time_minutes"].append(
                {"id": analysis.id, "value": analysis.reading_time_minutes or 0}
            )
    ...
    def _compare_sentiment(self, analyses: list[Analysis]) -> dict[str, Any]:
    ...
    def _compare_content(self, analyses: list[Analysis]) -> dict[str, Any]:
        ...
        for analysis in analyses:
            if analysis.word_frequency:
                word_frequencies.append({"id": analysis.id, "words": analysis.word_frequency})

            if analysis.concepts_detected:
                concepts_lists.append({"id": analysis.id, "concepts": analysis.concepts_detected})
    ...
    def _compare_readability(self, analyses: list[Analysis]) -> dict[str, Any]:
        """Compare les métriques de lisibilité"""
        readability_comparison: dict[str, Any] = {
            "flesch_ease_data": [],
            "flesch_kincaid_data": [],
            "statistics": {},
            "readability_levels": {},
        ...
        for analysis in analyses:
            readability_comparison["flesch_ease_data"].append(
                {
                    ...
                    "level": self._get_readability_level(analysis.flesch_reading_ease)
            ...
            readability_comparison["flesch_kincaid_data"].append(
                {"id": analysis.id, "grade": analysis.flesch_kincaid_grade}
            )
    ...
    def _calculate_content_similarity(self, analysis1: Analysis, analysis2: Analysis) -> float:
        ...
        try:
            ...
            if (
                analysis1.sentiment_polarity is not None
                and analysis2.sentiment_polarity is not None
            ):
            ...
            if analysis1.total_words and analysis2.total_words:
                words_ratio = min(analysis1.total_words, analysis2.total_words) / max(
                    analysis1.total_words, analysis2.total_words
                )
    ...
    def _generate_differential_insights(self, analyses: list[Analysis]) -> dict[str, Any]:
        """Génère des insights différentiels"""
        insights: dict[str, Any] = {"key_differences": [], "patterns": [], "recommendations": []}
        ...
        try:
            ...
            sentiment_values = [
                a.sentiment_polarity or 0 for a in analyses if a.sentiment_polarity is not None
            ]
            ...
            if len(insights["key_differences"]) > 0:
                insights["recommendations"].append(
                    "Analyser les causes des différences importantes identifiées"
                )

            if len(analyses) > 2:
                insights["recommendations"].append(
                    "Considérer une analyse de corrélation pour identifier les relations entre métriques"
                )
