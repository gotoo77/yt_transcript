...
@pytest.mark.parametrize(
    ("changes", "expected"),
    [
        ({}, {"complexity:very_complex", "sentiment:very_negative",
              "length:very_long", "readability:very_difficult", "source:text"}),
        ({"complexity_score": 0.1, "sentiment_polarity": 0.8,
          "total_words": 100, "flesch_reading_ease": 95, "video_id": "dQw4w9WgXcQ"},
         {"complexity:very_simple", "sentiment:very_positive",
          "length:very_short", "readability:very_easy", "source:youtube"}),
        ({"complexity_score": None, "sentiment_polarity": None, "total_words": None,
          "flesch_reading_ease": None, "created_at": None},
...
def test_filtered_tags_limit_and_missing_values(monkeypatch):
    records = [analysis(id=1), analysis(id=2, sentiment_polarity=0.8),
               analysis(id=3, sentiment_polarity=-0.9)]
    monkeypatch.setattr("yt_transcript.tags_service.get_recent_analyses",
...
def test_suggest_tags_matches_content_and_context(monkeypatch):
    ...
    monkeypatch.setattr("yt_transcript.tags_service.get_analysis_by_id",
                        lambda analysis_id: record)
    result = TagsService().suggest_tags(1)
    assert set(result["content_suggestions"]) == {
        "theme:technology", "theme:education", "theme:business",
        "theme:science", "theme:health",
    }
    assert set(result["contextual_suggestions"]) == {
        "format:long_form", "quality:rich_vocabulary", "level:advanced",
    }
