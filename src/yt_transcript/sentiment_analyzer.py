...
def analyze_sentiment(text: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        for sentence_text in re.split(r"[.!?]+", text):
            sentence = TextBlob(sentence_text.strip())
...
def detect_emotions(text: str) -> dict[str, Any] | None:
    """
    ...
    try:
        ...
        # Émotion dominante
        dominant_emotion = (
            max(emotion_scores.items(), key=lambda x: x[1]) if total_emotion_words > 0 else None
        )

        return {
            "emotions": emotion_percentages,
            "dominant_emotion": dominant_emotion[0]
            if dominant_emotion and dominant_emotion[1] > 0
            else None,
            "total_emotion_words": total_emotion_words,
            "emotional_intensity": round(total_emotion_words / len(text.split()) * 100, 1)
            if text
            else 0,
        }
...
def calculate_readability_metrics(text: str) -> dict[str, Any] | None:
    """
