...
from .database import Analysis, get_db_session, get_recent_analyses
...
class DashboardService:
    ...
    def get_global_kpis(self, days: int = 30) -> dict[str, Any] | None:
        ...
        try:
            ...
            words_result = (
                db.query(func.sum(Analysis.total_words))
                .filter(Analysis.created_at >= since_date, Analysis.total_words.isnot(None))
                .scalar()
                or 0
            )
            ...
            reading_time_result = (
                db.query(func.sum(Analysis.reading_time_minutes))
                .filter(
                    Analysis.created_at >= since_date, Analysis.reading_time_minutes.isnot(None)
                )
                .scalar()
                or 0
            )
            ...
            avg_complexity = (
                db.query(func.avg(Analysis.complexity_score))
                .filter(Analysis.created_at >= since_date, Analysis.complexity_score.isnot(None))
                .scalar()
                or 0
            )
            ...
            avg_sentiment = (
                db.query(func.avg(Analysis.sentiment_polarity))
                .filter(Analysis.created_at >= since_date, Analysis.sentiment_polarity.isnot(None))
                .scalar()
                or 0
            )
            ...
            sentiment_counts = (
                db.query(Analysis.sentiment_label, func.count(Analysis.id))
                .filter(Analysis.created_at >= since_date, Analysis.sentiment_label.isnot(None))
            ...
            # Analyses par mode
            mode_counts = (
                db.query(Analysis.analysis_mode, func.count(Analysis.id))
                .filter(Analysis.created_at >= since_date)
                .group_by(Analysis.analysis_mode)
                .all()
            )
            ...
            kpis = {
                ...
                "totals": {
                    ...
                    "reading_time_hours": round(reading_time_result / 60, 2)
                    if reading_time_result
                    else 0,
                },
                "averages": {
    ...
    def get_temporal_trends(self, days: int = 30) -> dict[str, Any] | None:
        ...
        try:
            ...
            # Filtrer par période
            period_analyses = [a for a in analyses if a.created_at and a.created_at >= since_date]

            if not period_analyses:
                return {"message": "Aucune donnée disponible pour la période"}

            # Grouper par jour
            daily_data: defaultdict[str, dict[str, Any]] = defaultdict(
                lambda: {
                    "analyses_count": 0,
                    "words_total": 0,
                    "complexity_scores": [],
            ...
            for date in sorted_dates:
                ...
                trends["daily_analyses"].append({"date": date, "count": day_data["analyses_count"]})

                trends["daily_words"].append({"date": date, "words": day_data["words_total"]})
    ...
    def get_content_insights(self, limit: int = 100) -> dict[str, Any] | None:
        ...
        try:
            ...
            insights = {
                "content_metrics": {
                    ...
                    "avg_vocabulary_richness": round(np.mean(vocab_richness), 2)
                    if vocab_richness
                    else 0,
    ...
    def _get_word_count_distribution(self, word_counts: list[int]) -> dict[str, Any]:
        """Distribue les comptes de mots par ranges"""
        if not word_counts:
            return {}

        ranges = {"0-500": 0, "501-1000": 0, "1001-2000": 0, "2001-5000": 0, "5000+": 0}
    ...
    def _get_complexity_distribution(self, complexities: list[float]) -> dict[str, Any]:
    ...
    def _get_sentiment_distribution(self, sentiments: list[float]) -> dict[str, Any]:
    ...
    def _get_top_analyses(self, analyses: list[Analysis]) -> dict[str, Any]:
        ...
        by_words = sorted(
            [a for a in analyses if a.total_words], key=lambda x: x.total_words or 0, reverse=True
        )[:3]

        by_complexity = sorted(
            [a for a in analyses if a.complexity_score],
            key=lambda x: x.complexity_score or 0,
            reverse=True,
        )[:3]

        by_vocabulary = sorted(
            [a for a in analyses if a.vocabulary_richness],
            key=lambda x: x.vocabulary_richness or 0,
            reverse=True,
        )[:3]

        return {
            "most_words": [
                {"id": a.id, "words": a.total_words, "created_at": a.created_at.isoformat()}
                for a in by_words
            ],
            "most_complex": [
                {
            ...
            "richest_vocabulary": [
                {
                    "id": a.id,
                    "richness": a.vocabulary_richness,
                    "created_at": a.created_at.isoformat(),
                }
    ...
    def _analyze_content_patterns(self, analyses: list[Analysis]) -> dict[str, Any]:
    ...
    def generate_comprehensive_dashboard(self, days: int = 30) -> dict[str, Any] | None:
