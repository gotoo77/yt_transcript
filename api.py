...
import logging
from database import get_recent_analyses, get_analysis_by_id, search_analyses
    ...
    class Analysis(Resource):
        ...
        def get(self, analysis_id):
            ...
            try:
                ...
                if not analysis:
                    api.abort(404, f"Analyse {analysis_id} non trouvée")
    ...
    class AnalysesSearch(Resource):
        ...
        def get(self):
            ...
            try:
                ...
                if not query:
                    api.abort(400, "Paramètre 'query' requis")
                ...
                for analysis in analyses:
                    result.append({
                        ...
                        'sentiment_label': analysis.sentiment_label,
                        'complexity_score': analysis.complexity_score,
                        'summary_text': analysis.summary_text[:150] + '...' if analysis.summary_text and len(analysis.summary_text) > 150 else analysis.summary_text
                    })
    ...
    class ExportJSON(Resource):
        ...
        def get(self, analysis_id):
            ...
            try:
                ...
                if not json_data:
                    api.abort(404, f"Analyse {analysis_id} non trouvée ou erreur d'export")
    ...
    class ExportCSV(Resource):
        ...
        def get(self):
            ...
            try:
                ...
                if not ids_param:
                    api.abort(400, "Paramètre 'ids' requis (ex: ids=1,2,3)")
                
                try:
                    analysis_ids = [int(x.strip()) for x in ids_param.split(',') if x.strip().isdigit()]
                except ValueError:
                    api.abort(400, "Format 'ids' invalide - utilisez des entiers séparés par des virgules")
                
                if not analysis_ids:
                    api.abort(400, "Aucun ID valide fourni")
                
                if len(analysis_ids) > 50:
                    api.abort(400, "Maximum 50 analyses par export CSV")
                
                csv_data = export_service.export_analysis_csv(analysis_ids)
                if not csv_data:
                    api.abort(500, "Erreur lors de la génération du CSV")
        ...
        def get(self):
            ...
            try:
                ...
                if not dashboard_data:
                    api.abort(500, "Erreur lors de la génération des données dashboard")
    ...
    class DashboardStats(Resource):
        ...
        def get(self):
            ...
            try:
                ...
                complexities = [a.complexity_score for a in analyses if a.complexity_score is not None]
